...
// ── getStagingPose() ──────────────────────────────────────────────────────────
// Returns the staging pose: the position behind the dock plus a combined yaw
// of (dock_yaw + staging_yaw_offset + kGuessingYawRad).
//
...
geometry_msgs::msg::PoseStamped GuessingDock::getStagingPose(
  ...
  const std::string & frame)
{
  ...
  double dock_yaw = 2.0 * std::atan2(dock_pose.orientation.z, dock_pose.orientation.w);
...
// ── getRefinedPose() ──────────────────────────────────────────────────────────
// No sensor — accept the dock pose supplied by the server as-is.
// Cache it in dock_pose_odom_ so isDocked() can use it.
bool GuessingDock::getRefinedPose(
...
// ── isDocked() ────────────────────────────────────────────────────────────────
// Transforms the latched dock pose into base_link and checks whether the
// 2-D Euclidean distance is within docking_threshold_.
bool GuessingDock::isDocked()
{
  std::lock_guard<std::mutex> lock(pose_mutex_);
  if (!dock_pose_received_) { return false; }

  try {
    geometry_msgs::msg::PoseStamped pose_base;
    tf_->transform(dock_pose_odom_, pose_base, base_frame_, tf2::durationFromSec(0.1));
...
PLUGINLIB_EXPORT_CLASS(
  lidar_dock_detector::GuessingDock,
